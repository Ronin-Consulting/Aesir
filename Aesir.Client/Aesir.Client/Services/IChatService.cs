...
{   
    ...
    Task<AesirChatResult> ChatCompletionsAsync(AesirChatRequestBase request);
    ...
    /// </returns>
    IAsyncEnumerable<AesirChatStreamedResult?> ChatCompletionsStreamedAsync(AesirChatRequestBase request);
    ...
    /// <returns>A task representing the asynchronous operation, with a result of type <see cref="AesirChatResult"/> containing the response from the API.</returns>
    Task<AesirChatResult> AgentChatCompletionsAsync(AesirAgentChatRequestBase request);
    ...
    /// Results include message deltas and metadata, and may yield null entries if no incremental data is provided.
    /// </returns>
    IAsyncEnumerable<AesirChatStreamedResult?> AgentChatCompletionsStreamedAsync(AesirAgentChatRequestBase request);
}