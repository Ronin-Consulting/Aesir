using Aesir.Modules.Research.Models;

namespace Aesir.Modules.Research.Services;

/// <summary>
/// Service interface for managing research team configurations.
/// Provides business logic layer between controllers and repositories.
/// </summary>
public interface IResearchTeamService
{
    /// <summary>
    /// Gets a research team by its unique identifier, including its members.
    /// </summary>
    /// <param name="id">The unique identifier of the research team.</param>
    /// <returns>The research team with members if found, null otherwise.</returns>
    Task<ResearchTeam?> GetByIdAsync(Guid id);

    /// <summary>
    /// Gets all research teams for a specific user.
    /// </summary>
    /// <param name="userId">The user identifier.</param>
    /// <returns>A collection of research teams for the user.</returns>
    Task<IEnumerable<ResearchTeam>> GetByUserIdAsync(string userId);

    /// <summary>
    /// Gets all active research teams for a specific user.
    /// </summary>
    /// <param name="userId">The user identifier.</param>
    /// <returns>A collection of active research teams for the user.</returns>
    Task<IEnumerable<ResearchTeam>> GetActiveByUserIdAsync(string userId);

    /// <summary>
    /// Creates a new research team with its members.
    /// </summary>
    /// <param name="team">The research team to create.</param>
    /// <returns>The created research team with its generated ID.</returns>
    Task<ResearchTeam> CreateAsync(ResearchTeam team);

    /// <summary>
    /// Updates an existing research team and its members.
    /// </summary>
    /// <param name="team">The research team to update.</param>
    /// <returns>The updated research team.</returns>
    Task<ResearchTeam> UpdateAsync(ResearchTeam team);

    /// <summary>
    /// Deletes a research team and all its members.
    /// </summary>
    /// <param name="id">The unique identifier of the research team to delete.</param>
    /// <returns>True if the deletion was successful, false otherwise.</returns>
    Task<bool> DeleteAsync(Guid id);

    /// <summary>
    /// Validates that all agent IDs referenced in team members exist.
    /// </summary>
    /// <param name="agentIds">The agent IDs to validate.</param>
    /// <returns>True if all agents exist, false otherwise.</returns>
    Task<bool> ValidateAgentIdsAsync(IEnumerable<Guid> agentIds);

    /// <summary>
    /// Duplicates an existing research team with a new name.
    /// </summary>
    /// <param name="id">The ID of the team to duplicate.</param>
    /// <param name="newName">The name for the duplicated team.</param>
    /// <returns>The newly created duplicate team.</returns>
    Task<ResearchTeam> DuplicateAsync(Guid id, string newName);
}
