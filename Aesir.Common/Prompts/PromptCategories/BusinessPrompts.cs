...
For general knowledge queries or responses not relying on `ChatDocSearch` documents, citations are not required unless explicitly referencing a specific document.
...
- Ensure all references to `ChatDocSearch` documents are accompanied by proper citations as specified above.  
...
- When using tools like `ChatDocSearch`, clearly indicate when the response is based on retrieved documents and provide citations accordingly.
");
}