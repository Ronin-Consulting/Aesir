...
{
    public static readonly PromptTemplate SystemPrompt = new(@"Generate a short, descriptive title for a conversation based on the user's message. The title should be 3-8 words that capture the main topic or intent. Use only plain text - no bullet points, dashes, or special formatting. The title should be suitable for display in a UI as a conversation label.
