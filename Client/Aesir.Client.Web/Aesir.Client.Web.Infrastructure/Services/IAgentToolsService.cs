...
public interface IAgentToolsService
{
    ...
    /// <summary>
    /// Gets the tools for an agent as ToolRequest objects ready for chat requests.
    /// Handles MCP server name resolution automatically.
    /// Results are cached per agent.
    /// </summary>
    /// <param name="agentId">The agent ID.</param>
    /// <param name="ct">Cancellation token.</param>
    /// <returns>List of ToolRequest objects for the agent's tools.</returns>
    Task<IReadOnlyList<ToolRequest>> GetAgentToolRequestsAsync(Guid agentId, CancellationToken ct = default);
